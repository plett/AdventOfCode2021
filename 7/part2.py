...
for i in range( min(pos) , max(pos) ):
    fuel = sum( [fuelcost(p,i) for p in pos] )
    #print(f"position {i} costs {fuel}")
    if bestfuel and fuel>bestfuel:
        #print("fuel costs rising, we are past minimum, abort")
        break
    if not bestfuel or fuel<bestfuel:
        bestfuel=fuel
        bestpos=i
print(f"Best postition is {bestpos}, it costs {bestfuel}")
